...
export default function App() {
  ...
  const removeJobIncomeField = (index) => {
    if (jobIncomes.length > 1) {
      const newIncomes = jobIncomes.filter((_, i) => i !== index);
      setJobIncomes(newIncomes);
    }
  };
  ...
  const InputField = ({ label, value, onChangeText, placeholder, keyboardType = 'numeric', multiline = false, icon }) => (
  ...
  const renderDeductionsTab = () => (
    <View style={styles.tabContent}>
      ...
      <InputField
        ...
        onChangeText={(v) => setDeductions({...deductions, workRelated: v})}
      ...
      <InputField
        ...
        onChangeText={(v) => setDeductions({...deductions, selfEducation: v})}
      ...
      <InputField
        ...
        onChangeText={(v) => setDeductions({...deductions, donations: v})}
      ...
      <InputField
        ...
        onChangeText={(v) => setDeductions({...deductions, other: v})}
