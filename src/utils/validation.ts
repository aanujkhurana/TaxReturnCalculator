 ...
 */
export const validateABN = (abn: string, required: boolean = false): ValidationResult => {
 ...
 */
export const validateFormStep = (formData: any, step: number): FormValidationResult => {
  ...
  switch (step) {
    case 1: // Income step
      ...
      if (formData.jobIncomes && formData.jobIncomes.length > 0) {
        const hasValidJobIncome = formData.jobIncomes.some((income: string) => {
